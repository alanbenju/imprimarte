    ...
    <>
      <KonvaImage
        ...
        dragBoundFunc={(pos) => {
          const newX = Math.max(
            rectX,
            Math.min(pos.x, rectX + rectWidth - imageRef.current!.width())
          );
          const newY = Math.max(
            rectY,
            Math.min(pos.y, rectY + rectHeight - imageRef.current!.height())
          );
        ...
        onTransformEnd={(e) => {
          ...
          if (node) {
            ...
            if (
              newWidthCM > maxLongSideCM ||
              newHeightCM > maxLongSideCM ||
